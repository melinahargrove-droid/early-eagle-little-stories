...
export async function buildBookPages(bookId) {
  ...
  pages.push({
    id: makeId('page'), bookId, order: order++, layoutId: 'cover',
    photoIds: photoIds.length ? [photoIds[0]] : [], title: book.title,
    caption: '', quote: '', speaker: '', createdAt: now, updatedAt: now,
  })
  ...
  while (cursor < photoIds.length) {
    ...
    pages.push({
      id: makeId('page'), bookId, order: order++, layoutId,
      photoIds: photoIds.slice(cursor, cursor + count),
      title: '', caption: '', quote: '', speaker: '', createdAt: now, updatedAt: now,
    })
    cursor += count
    flowIndex += 1
  }
  ...
  pages.push({
    id: makeId('page'), bookId, order: order++, layoutId: 'the-end',
    photoIds: [], title: 'The End', caption: '', quote: '', speaker: '', createdAt: now, updatedAt: now,
  })
...
export async function updatePageLayout(pageId, layoutId) {
  await db.pages.update(pageId, { layoutId, updatedAt: new Date().toISOString() })
...
export async function tryAnotherLook(pageId) {
  ...
  const options = count <= 1
    ? ['one-photo', 'photo-caption', 'photo-quote']
    : count === 2
      ? ['two-equal', 'one-large-one-small', 'two-stacked']
      : count === 3
        ? ['three-story', 'collage-3']
        : ['four-grid', 'collage-4']
