...
export default function App() {
  ...
  return <main className="app-shell home-shell"><section className="home-screen"><header className="brand-header home-brand"><p className="eyebrow">One Little Teacher</p><h1>Little Stories</h1><p className="tagline">our classroom stories</p></header><button className="create-story-card illustrated-create" type="button" onClick={() => setStep('name')}><img src={`${ASSET_BASE}little-stories-create-book-camera.png`} alt="" className="create-camera-art"/><div className="create-story-copy"><span className="create-plus">＋</span><strong>Create a Book</strong><small>Turn today's moments into a beautiful book.</small></div></button><section className="library-section home-library"><div className="section-heading-row"><div><p className="section-kicker">Your classroom bookshelf</p><h2>Your Books</h2></div><span className="book-count">{books.length}</span></div>{books.length === 0 ? <div className="empty-state"><div className="book-placeholder"><span>Little<br/>Stories</span></div><h3>Your first story starts here.</h3><p>Add a folder of classroom photos and we'll turn them into a beautiful first draft.</p></div> : <div className="books-grid">{books.map((book) => <HomeBookCard book={book} onOpen={() => openBook(book)} key={book.id} />)}</div>}</section><footer className="backup-bar illustrated-backup"><img src={`${ASSET_BASE}little-stories-cloud-saved-icon.png`} alt=""/><div><strong>Saved & backed up</strong><span>{storageState}</span></div></footer></section></main>
}
...
function Editor({ book, page, pageIndex, pages, onClose, onPreview, onSelectPage, onReorder, onPrev, onNext, onLayout, onAnother, onText, onPhotos }) {
  ...
  if (!page) return null
  return <main className="app-shell" data-book-theme={book.themeId}><section className="editor-screen"><header className="editor-header"><button className="close-button" onClick={onClose}>×</button><div><strong>{book.title}</strong><span>Page {pageIndex + 1} of {pages.length} • Saved</span></div><button className="preview-button" type="button" onClick={onPreview}>Preview</button></header><div className={`page-canvas layout-${page.layoutId}`} style={themeVars(book.themeId)}><div className="theme-decoration theme-decoration-a">{getTheme(book.themeId).motif}</div><div className="theme-decoration theme-decoration-b">{getTheme(book.themeId).motif}</div><div className="page-title">{page.title}</div><div className="photo-slots">{urls.map((url, index) => <img src={url} alt="" key={url + index}/>)}</div>{page.caption && <p className="page-caption">{page.caption}</p>}{page.quote && <figure className="page-quote"><blockquote>“{page.quote}”</blockquote>{page.speaker && <figcaption>— {page.speaker}</figcaption>}</figure>}{page.layoutId === 'the-end' && <div className="end-title">The End</div>}</div><div className="page-nav"><button onClick={onPrev} disabled={pageIndex === 0}>← Previous</button><button className="all-pages-button" onClick={() => setShowPages(true)}>All Pages</button><button onClick={onNext} disabled={pageIndex === pages.length - 1}>Next →</button></div><div className="editor-tools"><button type="button" on