...
function Editor({ book, page, pageIndex, pages, onClose, onPreview, onSelectPage, onReorder, onPrev, onNext, onLayout, onAnother, onText, onPhotos }) {
  ...
  if (!page) return null
  return <main className="app-shell"><section className="editor-screen"><header className="editor-header"><button className="close-button" onClick={onClose}>×</button><div><strong>{book.title}</strong><span>Page {pageIndex + 1} of {pages.length} • Saved</span></div><button className="preview-button" type="button" onClick={onPreview}>Preview</button></header><div className={`page-canvas layout-${page.layoutId}`}><div className="page-title">{page.title}</div><div className="photo-slots">{urls.map((url, index) => <img src={url} alt="" key={url + index}/>)}</div>{page.caption && <p className="page-caption">{page.caption}</p>}{page.quote && <figure className="page-quote"><blockquote>“{page.quote}”</blockquote>{page.speaker && <figcaption>— {page.speaker}</figcaption>}</figure>}{page.layoutId === 'the-end' && <div className="end-title">The End</div>}</div><div className="page-nav"><button onClick={onPrev} disabled={pageIndex === 0}>← Previous</button><button className="all-pages-button" onClick={() => setShowPages(true)}>All Pages</button><button onClick={onNext} disabled={pageIndex === pages.length - 1}>Next →</button></div><div className="editor-tools"><button type="button" onClick={openPhotos}>Photos</button><button type="button" onClick={() => setShowLayouts((v) => !v)}>Layout</button><button type="button" onClick={() => setShowText(true)}>Caption</button><button type="button" onClick={() => setShowPages(true)}>More</button></div>{showLayouts && <div className="layout-sheet"><strong>Change Layout</strong><div>{LAYOUTS.map((layout) => <button type="button" key={layout} onClick={() => { onLayout(layout); setShowLayouts(false) }}>{layout.replaceAll('-', ' ')}</button>)}</div></div>}{!['cover','the-end'].includes(page.layoutId) && <button className="another-look" type="button" onClick={onAnother}>✨ Try Another Look</button>}

    {showText && <div className="sheet-backdrop" onClick={() => setShowText(false)}><section className="bottom-sheet" onClick={(e) => e.stopPropagation()}><h3>Add words to this page</h3><label>Caption<textarea defaultValue={page.caption} placeholder="A short caption" id="caption-text"/></label><label>Child quote<textarea defaultValue={page.quote} placeholder="What did a child say?" id="quote-text"/></label><label>Child name<input defaultValue={page.speaker} placeholder="Optional" id="speaker-text"/></label><button className="primary-action compact" type="button" onClick={() => { onText({ caption: document.getElementById('caption-text').value, quote: document.getElementById('quote-text').value, speaker: document.getElementById('speaker-text').value }); setShowText(false) }}>Save Text</button></section></div>}

    {showPhotos && <div className="sheet-backdrop" onClick={() => setShowPhotos(false)}><section className="bottom-sheet" onClick={(e) => e.stopPropagation()}><h3>Choose photos for this page</h3><p className="sheet-copy">Tap photos to replace this page’s current selection. Nothing is deleted from the book library.</p><div className="photo-library-grid">{library.map((item) => <button className={(page.photoIds ?? []).includes(item.id) ? 'selected-photo' : ''} type="button" key={item.id} onClick={() => { const max = page.layoutId.includes('four') || page.layoutId.includes('collage') ? 4 : page.layoutId.includes('three') ? 3 : page.layoutId.includes('two') || page.layoutId.includes('large-one-small') ? 2 : 1; const current = [...(page.photoIds ?? [])]; const exists = current.includes(item.id); let next = exists ? current.filter((id) => id !== item.id) : [...current, item.id].slice(-max); onPhotos(next) }}><img src={item.url} alt=""/></button>)}</div><button className="primary-action compact" type="button" onClick={() => setShowPhotos(false)}>Done</button></section></div>}

    {showPages && <div className="sheet-backdrop" onClick={() => setShowPages(false)}><section className="bottom-sheet pages-sheet" onClick={(e) => e.stopPropagation()}><h3>All Pages</h3><p className="sheet-copy">Open any page, or move inside pages up and down. Cover and The End stay anchored.</p><div className="page-list">{pages.map((item, index) => <div className={`page-row ${index === pageIndex ? 'current' : ''}`} key={item.id}><button className="page-open" type="button" onClick={() => { onSelectPage(index); setShowPages(false) }}><span>{index + 1}</span><strong>{item.layoutId.replaceAll('-', ' ')}</strong></button><div><button type="button" onClick={() => movePage(index, -1)} disabled={index <= 1}>↑</button><button type="button" onClick={() => movePage(index, 1)} disabled={index === 0 || index >= pages.length - 2}>↓</button></div></div>)}</div></section></div>}
  </section></main>
}
